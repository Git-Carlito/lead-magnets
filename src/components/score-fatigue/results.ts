import { OPTION_TYPE_MAP, QUESTIONS } from "./questions";
import type { ActiveType, FatigueResult } from "./types";

export const RESULTS: Record<string, FatigueResult> = {
  physical: {
    key: "physical",
...
export function calculateResult(answers: Record<string, string>): FatigueResult {
  ...
  for (const answerValue of Object.values(answers)) {
    const type = OPTION_TYPE_MAP.get(answerValue);
    if (type && type !== "neutral") {
      scores[type as ActiveType]++;
    }
  }
